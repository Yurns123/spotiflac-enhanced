...
// --- Streaming & Preloading FFI Exports ---

func StartStreamDownloadJSON(id, urlStr, outputPath string, streamableThreshold int64) (string, error) {
	sd, err := StartStreamDownload(id, urlStr, outputPath, streamableThreshold)
	if err != nil {
		return "", err
	}
...
func GetStreamStateJSON(id string) string {
	return toJSON(GetStreamState(id))
}

func IsStreamableJSON(id string) bool {
	return IsStreamable(id)
}

func IsStreamCompleteJSON(id string) bool {
	return IsStreamComplete(id)
}

func CancelStreamJSON(id string) {
	CancelStream(id)
}

func CleanupStreamJSON(id string) {
	CleanupStream(id)
}

func EnqueuePreloadJSON(itemID, trackName, artistName, outputPath, downloadURL string, priority int) (string, error) {
	err := EnqueuePreload(itemID, trackName, artistName, outputPath, downloadURL, priority)
	if err != nil {
		return "", err
	}
...
func GetPreloadStatusJSON() string {
	return GetPreloadStatus()
}

func CancelPreloadJSON() {
	CancelPreload()
}

func IsPreloadCompleteJSON(itemID string) bool {
	return IsPreloadComplete(itemID)
}
